...
# ================== (YENİ) Özet Öncesi İyileştirici ==================
NOISE_PATTERNS = [
    r"\[(music|applause|laughter|laughs|claps|cheer|silence)\]",
    r"\((music|applause|laughter|laughs|claps|cheer|silence)\)",
    r"[🎵🎶👏😂🤣🙄🤔]+",
    r"https?://\S+",
]

FILLERS_TR = r"\b(şey|yani|hani|aslında|işte|gibi|falan|filan|ı+|i+|ıı+|eee+|ee+)\b"
FILLERS_EN = r"\b(uh|um+|erm+|you know|like|i mean|kind of|sort of|basically|actually)\b"
...
def _strip_noise(s: str) -> str:
    ...
    x = re.sub(r"\[[^\]\n]{1,20}\]", " ", x)
    x = re.sub(r"(?<!\w)[^\w\s.,!?]+(?!\w)", " ", x)
    return x

def _remove_fillers(s: str, lang_hint: str) -> str:
    x = s
    if lang_hint == "turkish":
        x = re.sub(FILLERS_TR, " ", x, flags=re.I)
    else:
        x = re.sub(FILLERS_EN, " ", x, flags=re.I)
    return re.sub(r"\s{2,}", " ", x).strip()

def _merge_micro_segments(segments: List[Dict], max_len_words: int = 4, max_dur: float = 2.0) -> List[Dict]:
    if not segments: return segments
    merged = []
    buf = None
    for s in segments:
        wcount = len((s.get("text") or "").split())
        dur = float(s.get("end",0))-float(s.get("start",0))
        if buf is None:
            buf = dict(s)
            continue
        if (wcount <= max_len_words) or (dur <= max_dur):
            buf["end"] = float(s.get("end", buf["end"]))
            buf["text"] = (buf.get("text","") + " " + (s.get("text","") or "")).strip()
        else:
            merged.append(buf); buf = dict(s)
    if buf: merged.append(buf)
    return merged

def _sentence_fixup(text: str) -> str:
    text = re.sub(r"\s+", " ", text).strip()
    text = re.sub(r"([^\.\!\?])\s+(?=[A-ZĞÜŞİÖÇ])", r"\1. ", text)
    def _cap(m):
        t = m.group(0)
        return t[0].upper() + t[1:]
    text = re.sub(r"(^|[\.!\?]\s+)([a-zğüşiöç])", lambda m: m.group(1)+m.group(2).upper(), text)
    return text

def _drop_low_content_lines(lines: List[str]) -> List[str]:
    out = []
    for line in lines:
        t = line.strip()
        if not t: continue
        letters = sum(c.isalpha() for c in t)
        if len(t) < 3 or (letters / max(1,len(t)) < 0.35):
            continue
        out.append(t)
    return out

def preprocess_transcript_for_summary(seg_list: List[Dict], lang_ui: str, use_diar: bool) -> str:
    """
    1) mikro-segment birleştirme
    2) noise strip
    3) filler temizliği
    4) cümle fix
    5) düşük içerikli satırları at
    6) diar varsa aynı konuşmacıyı blokla
    """
    segs = _merge_micro_segments(seg_list)

    cleaned = []
    for s in segs:
        txt = ensure_str(s.get("text",""))
        txt = _strip_noise(txt)
        txt = _remove_fillers(txt, lang_ui)
        txt = collapse_repeats(txt, max_repeat=1)
        if txt:
            ss = dict(s); ss["text"] = txt
            cleaned.append(ss)
    ...
    lines = []
    if use_diar and any(c.get("speaker") for c in cleaned):
        current_spk = None
        buf = []
        for s in cleaned:
            spk = s.get("speaker")
            if spk != current_spk:
                if buf:
                    ...
                    block = _sentence_fixup(block)
                    lines.append(block)
                    buf = []
                current_spk = spk
                if spk: lines.append(f"{spk}:")
            buf.append(s["text"])
        if buf:
            block = " ".join(buf).strip()
            block = _sentence_fixup(block)
        ...
        lines = [_sentence_fixup(joined)]

    lines = _drop_low_content_lines(lines)
    out = "\n".join(lines)
    out = clean_text(out)
    return out
...
def do_summarize():
    ...
    # ---- YENİ: Özet öncesi girdi iyileştirme ----
    pre = preprocess_transcript_for_summary(ss.seg_list, lang_ui=sum_lang, use_diar=enable_diar)
    if not pre.strip():
        st.warning("Özet öncesi temizlikten sonra içerik bulunamadı.")
        return

    chunks = chunk_text_mode(pre, length_mode)
